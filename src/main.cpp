...
#if defined(NDEBUG)
#error "BITCREXCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubic) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zBIC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zBIC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zBIC spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zBIC without the correct spend type. txid=%s\n", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zBIC spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZBICModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZBICModule::validateInput(txin, prevOut, tx, ret)) {
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (txout.scriptPubKey.IsPayToColdStaking()) {
            ...
            if (txout.nValue < minColdStakingAmount)
                return state.DoS(100, error("%s: dust amount (%d) not allowed for cold staking. Min amount: %d", __func__, txout.nValue, minColdStakingAmount), REJECT_INVALID, "bad-txns-cold-stake");
        }
    }
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
        return state.DoS(10, error("%s : Zerocoin transactions are temporarily disabled for maintenance", __func__), REJECT_INVALID, "bad-tx");
    ...
    if (tx.IsCoinStake())
        return state.DoS(100, error("%s : coinstake as individual tx (id=%s): %s", __func__, tx.GetHash().GetHex(), tx.ToString()), REJECT_INVALID, "coinstake");
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zBIC spend tx %s already in block %d",
                                         __func__, tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
                        ...
                        REJECT_INVALID, "bad-txns-invalid-zbic");
                }
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZBICModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)) {
                        return false;
                    }
                    ...
                    if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                        return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                                                 __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()),
                            REJECT_INVALID, "bad-txns-invalid-zbic");
                    }
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (!ValidOutPoint(txin.prevout, chainActive.Height())) {
                    return state.Invalid(error("%s : tried to spend invalid input %s in tx %s",
                                             __func__, txin.prevout.ToString(), tx.GetHash().GetHex()),
                        REJECT_INVALID, "bad-txns-invalid-inputs");
                }
            }

            // Check that zBIC mints (if included) are not already known
        ...
        if (!hasZcSpendInputs) {
            ...
            if (nSigOps > nMaxSigOps)
                return state.DoS(0, error("%s : too many sigops %s, %d > %d", __func__, hash.ToString(), nSigOps, nMaxSigOps), REJECT_NONSTANDARD, "bad-txns-too-many-sigops");
        }
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            ...
            if (fLimitFree && nFees < txMinFee && !hasZcSpendInputs)
                return state.DoS(0, error("%s : not enough fees %s, %d < %d", __func__, hash.ToString(), nFees, txMinFee), REJECT_INSUFFICIENTFEE, "insufficient fee");
            ...
            if (fLimitFree && nFees < ::minRelayTxFee.GetFee(nSize) && !hasZcSpendInputs) {
                ...
                if (dFreeCount >= GetArg("-limitfreerelay", 30) * 10 * 1000)
                    return state.DoS(0, error("%s : free transaction rejected by rate limiter", __func__), REJECT_INSUFFICIENTFEE, "rate limited free transaction");
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 1) {
        nSubsidy = 50000 * COIN;
    } else if (nHeight >= 1 && nHeight <= 15000) {
        nSubsidy = 0.06 * COIN;
        ...
        nSubsidy = 0.5 * COIN;
    } else if (nHeight > 75000 && nHeight <= 85000) {
        nSubsidy = 0.65 * COIN;
    } else if (nHeight > 85000 && nHeight <= 95000) {
        nSubsidy = 0.85 * COIN;
    } else if (nHeight > 95000 && nHeight <= 105000) {
        nSubsidy = 1 * COIN;
    } else if (nHeight > 105000 && nHeight <= 115000) {
        nSubsidy = 1.2 * COIN;
    } else if (nHeight > 115000 && nHeight <= 125000) {
        nSubsidy = 1.3 * COIN;
    } else if (nHeight > 125000 && nHeight <= 135000) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight > 135000 && nHeight <= 145000) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight > 145000 && nHeight <= 155000) {
        nSubsidy = 1.6 * COIN;
    } else if (nHeight > 155000 && nHeight <= 165000) {
        nSubsidy = 1.7 * COIN;
    } else if (nHeight > 165000 && nHeight <= 175000) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight > 175000 && nHeight <= 185000) {
        nSubsidy = 1.9 * COIN;
    } else if (nHeight > 185000 && nHeight <= 195000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight > 195000 && nHeight <= 205000) {
        nSubsidy = 1.55 * COIN;
    } else if (nHeight > 205000 && nHeight <= 215000) {
        nSubsidy = 1.25 * COIN;
    } else if (nHeight > 215000 && nHeight <= 235000) {
        nSubsidy = 0.95 * COIN;
    } else if (nHeight > 235000 && nHeight <= 245000) {
        nSubsidy = 0.55 * COIN;
    } else if (nHeight > 245000 && nHeight <= 265000) {
        nSubsidy = 0.35 * COIN;
    } else if (nHeight > 265000 && nHeight <= 500000) {
        nSubsidy = 0.22 * COIN;
    } else if (nHeight > 500000 && nHeight <= 900000) {
        nSubsidy = 0.19 * COIN;
    } else if (nHeight > 900000 && nHeight <= 1500000) {
        nSubsidy = 0.17 * COIN;
    } else if (nHeight > 1500000 && nHeight <= 5500000) {
        nSubsidy = 0.08 * COIN;
    } else
        nSubsidy = 0.05 * COIN;
...
int64_t GetDevFundPayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight == 0) {
        ...
        ret_val = blockValue - 0.3 * COIN; //99.99% of the reward while POS
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBICStake)
{
    int64_t ret = 0;
    ...
    ret = blockValue * 90 / 100;     // 90% of block value
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZBICModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)) {
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase() && !tx.HasZerocoinSpendInputs()) {
        ...
        // Only if ALL inputs pass do we perform expensive BICSA signature checks.
        // Helps prevent CPU exhaustion attacks.

        // Skip BICSA signature verification when connecting blocks
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BITCREXCOIN
...
{
    RenameThread("bitcrexcoin-scriptch");
...
void RecalculateZBICMinted()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZBIC..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZBIC..."), percent);
        }
...
void RecalculateZBICSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZBIC..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZBIC..."), percent);
        }
        ...
        //Remove spends from zBIC supply
...
bool RecalculateBICSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating BIC supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating BIC supply..."), percent);
        }
...
{
    // BITCREXCOIN: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZBICSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zBIC supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == Params().Zerocoin_Block_EndFakeSerial() + 1 && pindex->GetZerocoinSupply() < Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount()) {
        for (auto denom : libzerocoin::zerocoinDenomList) {
            pindex->mapZerocoinSupply.at(denom) += GetWrapppedSerialInflation(denom);
        }
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zBIC mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZBICMinted();
        RecalculateZBICSpent();
        RecalculateBICSupply(Params().Zerocoin_StartHeight());
    }

    //Track zBIC money supply in the block index
    if (!UpdateZBICSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zBIC supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert BIC to zBIC
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >= 3 && lastOut.scriptPubKey != tx.vout[outs - 2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 BIC.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBIC spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if (!IsInitialBlockDownload() && !CheckPublicCoinSpendVersion(spend.getVersion())) {
                            return state.DoS(0, error("%s : Public Zerocoin spend version %d not accepted. must be version %d.", __func__, spend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-zcspend-version");
                        }
                    } else {
                        spend = TxInToZerocoinSpend(txIn);
                    }
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBIC serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // BitcrexCoin Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t)Params().BitcrexCoinBadBlockTime()) && (block.nBits == (uint32_t)Params().BitcrexCoinBadBlocknBits())) {
            // accept BITCREXCOIN block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bicInputs;
        std::vector<CTxIn> zBICInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if (stakeIn.IsZerocoinSpend()) {
                zBICInputs.push_back(stakeIn);
            } else {
                bicInputs.push_back(stakeIn);
            }
        }
        const bool hasBICInputs = !bicInputs.empty();
        const bool hasZBICInputs = !zBICInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in : tx.vin) {
                ...
                if (hasBICInputs)
                    ...
                    for (const CTxIn& bicIn : bicInputs) {
                        if (bicIn.prevout == in.prevout) {
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in : t.vin) {
                        ...
                        for (const CTxIn& stakeIn : bicInputs) {
            ...
            // Now that this loop if completed. Check if we have zBIC inputs.
            if (hasZBICInputs) {
                for (const CTxIn& zBicInput : zBICInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zBicInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbic");
            ...
            if (!isBlockFromFork)
                for (const CTxIn& zBicInput : zBICInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpend(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100, error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbic");
                }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BITCREXCOIN: We use certain sporks during IBD, so check to see if they are
        ...
        if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zbic", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zbic", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
