...
{
    return strprintf("CTxOut(nValue=%d.%04d, scriptPubKey=%s)", nValue / COIN, nValue % COIN, scriptPubKey.ToString().substr(0,30));
}
...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between bic and zbic
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // BITCREXCOIN: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
