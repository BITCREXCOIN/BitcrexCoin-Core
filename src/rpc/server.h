...
/**
 * BITCREXCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZbicSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
// extern UniValue setzbicseed(const UniValue& params, bool fHelp);
// extern UniValue getzbicseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
// extern UniValue searchdzbic(const UniValue& params, bool fHelp);
// extern UniValue dzbicstate(const UniValue& params, bool fHelp);
