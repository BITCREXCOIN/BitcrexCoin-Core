...
CzBICWallet::CzBICWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbic seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBICSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBICSeed_deprecated()) {
                    LogPrintf("%s: Updated zBIC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbic seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbic wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBICWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBICCount(nCountLastUsed);
    else if (!walletdb.ReadZBICCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBICWallet::Lock()
{
    seedMaster = 0;
}

void CzBICWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBICWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBIC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBICWallet::LoadMintPoolFromDB()
{
...
void CzBICWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBICWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBICWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbicTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBICWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBIC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbicTracker which also adds to database
    pwalletMain->zbicTracker->Add(dMint, true);
...
void CzBICWallet::SeedToZBIC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBICWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBICWallet::GenerateDeterministicZBIC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBICWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBICWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBICWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
