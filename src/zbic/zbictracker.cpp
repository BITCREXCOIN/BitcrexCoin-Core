// Copyright (c) 2018-2019 The BITCREXCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zbic/deterministicmint.h>
#include "zbictracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzBICTracker::CzBICTracker(std::string strWalletFile)
{
...
CzBICTracker::~CzBICTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBICTracker::Init()
{
...
bool CzBICTracker::Archive(CMintMeta& meta)
{
...
bool CzBICTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBICTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBICTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBICTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBICTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBICTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBICTracker::GetSerialHashes()
{
...
CAmount CzBICTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBICTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBICTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBICTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBICTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBICTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBICTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBICTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBICTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBICTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBICTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBICWallet* zBICWallet)
{
    bool iszBICWalletInitialized = (NULL != zBICWallet);
    CMintMeta meta;
    ...
    if (! iszBICWalletInitialized)
        zBICWallet = new CzBICWallet(strWalletFile);
    meta.isSeedCorrect = zBICWallet->CheckSeed(dMint);
    if (! iszBICWalletInitialized)
        delete zBICWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBICTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBICTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBICTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBICTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBICTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBICTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBICWallet* zBICWallet = new CzBICWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBICWallet);
        }
        delete zBICWallet;
        LogPrint("zero", "%s: added %d dzbic from DB\n", __func__, listDeterministicDB.size());
    }
