...
#include "ui_zbiccontroldialog.h"
...
std::set<std::string> ZBicControlDialog::setSelectedMints;
std::set<CMintMeta> ZBicControlDialog::setMints;

bool CZBicControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBicControlDialog::COLUMN_DENOMINATION || column == ZBicControlDialog::COLUMN_VERSION || column == ZBicControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBicControlDialog::ZBicControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBicControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBIC Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZBic->setProperty("cssClass", "text-main-purple");
    ui->labelZBic_int->setProperty("cssClass", "text-main-purple");
...
ZBicControlDialog::~ZBicControlDialog()
{
    delete ui;
}

void ZBicControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBicControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBicControlWidgetItem* itemDenom(new CZBicControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBicControlWidgetItem *itemMint = new CZBicControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zbicTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbicTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBIC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBIC seed used to mint this zBIC is not the same as currently hold in the wallet";
            else
...
void ZBicControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBicControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBic_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZBicControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBicControlDialog::GetSelectedMints()
{
...
void ZBicControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
