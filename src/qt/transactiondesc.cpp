...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    CAmount nNet = rec->bic + rec->debit;
    ...
    // Amount
    //
    if (wtx.IsCoinBase() && rec->bic == 0) {
        //
        // Coinbase
        //
        CAmount nUnmatured = 0;
        for (const CTxOut& txout : wtx.vout)
            nUnmatured += wallet->GetBic(txout, ISMINE_ALL);
        strHTML += "<b>" + tr("Bic") + ":</b> ";
        ...
        // Bic
        //
        strHTML += "<b>" + tr("Bic") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const CTxOut& txout : wtx.vout) {
                ...
                if (toSelf)
                    strHTML += "<b>" + tr("Bic") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }

            if (fAllToMe) {
                // Payment to self
                CAmount nChange = wtx.GetChange();
                CAmount nValue = rec->bic - nChange;
                strHTML += "<b>" + tr("Total debit") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total bic") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }
            ...
            for (const CTxOut& txout : wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Bic") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, wallet->GetBic(txout, ISMINE_ALL)) + "<br>";
        }
    }
    ...
    // Message from normal bitcrexcoin:URI (bitcrexcoin:XyZ...?message=example)
