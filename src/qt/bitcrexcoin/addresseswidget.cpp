...
#include "qt/bitcrexcoin/forms/ui_addresseswidget.h"
#include "qt/bitcrexcoin/addresslabelrow.h"
#include "qt/bitcrexcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(BITCREXCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a BITCREXCOIN address"));
    setCssProperty(ui->labelAddress, "text-title");
    ui->lineEditAddress->setPlaceholderText("e.g. E7VFR83SQbiezrW72hjc…");
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress bicAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(bicAdd.Get(), label.toUtf8().constData(),
                bicAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
