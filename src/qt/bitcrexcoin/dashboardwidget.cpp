...
#include "qt/bitcrexcoin/dashboardwidget.h"
#include "qt/bitcrexcoin/forms/ui_dashboardwidget.h"
#include "qt/bitcrexcoin/sendconfirmdialog.h"
#include "qt/bitcrexcoin/txrow.h"
...
DashboardWidget::DashboardWidget(BITCREXCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of BIC staked."));  // Amount of BIC and zBIC staked.
    setCssSubtitleScreen(ui->labelMessage);
    // ui->labelMessage->setVisible(false);
    
    setCssProperty(ui->labelSquareBic, "square-chart-bic");
    // setCssProperty(ui->labelSquarezBic, "square-chart-zbic");
    setCssProperty(ui->labelBic, "text-chart-bic");
    // setCssProperty(ui->labelZbic, "text-chart-zbic");

    // ui->labelBic->setVisible(false);
    // ui->labelZbic->setVisible(false);
    ...
    ui->labelAmountZbic->setText("0 zBIC");
    ui->labelAmountBic->setText("0 BIC");
    setCssProperty(ui->labelAmountBic, "text-stake-bic-disable");
    setCssProperty(ui->labelAmountZbic, "text-stake-zbic-disable");
    ui->labelAmountZbic->setVisible(false);
    ...
    ui->btnHowTo->setText(tr("How to get BIC or zBIC"));
...
void DashboardWidget::changeChartColors(){
    ...
    if(isLightTheme()){
        ...
        gridLineColorX = QColor(11,22,21);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isBic = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZBIC;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair BIC, zBIC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bic = (pair.first != 0) ? pair.first / 100000000 : 0;
            zbic = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalBic += pair.first;
            chartData->totalZbic += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBic.append(bic);
        chartData->valueszBic.append(zbic);

        int max = std::max(bic, zbic);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("BIC");
    set1 = new QBarSet("zBIC");
    set0->setColor(QColor(75,119,125));
    set1->setColor(QColor(136,237,255));
    ...
    set0->append(chartData->valuesBic);
    set1->append(chartData->valueszBic);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBic > 0 || chartData->totalZbic > 0) {
        setCssProperty(ui->labelAmountBic, "text-stake-bic");
        setCssProperty(ui->labelAmountZbic, "text-stake-zbic");
    } else {
        setCssProperty(ui->labelAmountBic, "text-stake-bic-disable");
        setCssProperty(ui->labelAmountZbic, "text-stake-zbic-disable");
    }
    forceUpdateStyle({ui->labelAmountBic, ui->labelAmountZbic});
    ui->labelAmountBic->setText(GUIUtil::formatBalance(chartData->totalBic, nDisplayUnit));
    ui->labelAmountZbic->setText(GUIUtil::formatBalance(chartData->totalZbic, nDisplayUnit, true));
