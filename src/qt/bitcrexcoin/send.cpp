...
#include "qt/bitcrexcoin/send.h"
#include "qt/bitcrexcoin/forms/ui_send.h"
#include "qt/bitcrexcoin/addnewcontactdialog.h"
#include "qt/bitcrexcoin/qtutils.h"
#include "qt/bitcrexcoin/sendchangeaddressdialog.h"
#include "qt/bitcrexcoin/optionbutton.h"
#include "qt/bitcrexcoin/sendconfirmdialog.h"
#include "qt/bitcrexcoin/myaddressrow.h"
#include "qt/bitcrexcoin/guitransactionsutils.h"
...
#include "zbiccontroldialog.h"

SendWidget::SendWidget(BITCREXCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("BIC");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zBIC");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BIC) or private coins (zBIC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a BITCREXCOIN address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BIC");
    ...
    setCssProperty(coinIcon, "coin-icon-bic");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onBICSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onBICSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZbic = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZbic));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZbic ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendBic = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendBic ? AskPassphraseDialog::Context::Send_BIC : AskPassphraseDialog::Context::Send_zBIC, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendBic) ? send(recipients) : sendZbic(recipients)) {
...
bool SendWidget::sendZbic(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zBIC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zBIC selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZBicControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBicControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZbic(
        ...
        inform(tr("zBIC transaction sent!"));
        ZBicControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZBIC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBIC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBIC transaction
        ...
        emit message("zBIC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isBIC){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BIC to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZBicControlDialog *zBicControl = new ZBicControlDialog(this);
            zBicControl->setModel(walletModel);
            zBicControl->exec();
            ui->btnCoinControl->setActive(!ZBicControlDialog::setSelectedMints.empty());
            zBicControl->deleteLater();
        } else {
            inform(tr("You don't have any zBIC in your balance to select."));
        }
    }
}
...
void SendWidget::onBICSelected(bool _isBIC){
    isBIC = _isBIC;
    setCssProperty(coinIcon, _isBIC ? "coin-icon-bic" : "coin-icon-zbic");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress bicAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(bicAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(bicAdd.Get(), dialog->getLabel().toStdString(), "send")) {
