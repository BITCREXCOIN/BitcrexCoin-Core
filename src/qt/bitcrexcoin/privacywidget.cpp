...
#include "qt/bitcrexcoin/privacywidget.h"
#include "qt/bitcrexcoin/forms/ui_privacywidget.h"
#include "qt/bitcrexcoin/qtutils.h"
#include "guiutil.h"
#include "qt/bitcrexcoin/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(BITCREXCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBIC anonymizes your BIC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBIC or convert back to BIC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BIC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBIC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBIC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBIC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBIC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBIC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBIC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBIC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBIC");
    ...
    // Only Convert to BIC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBIC->setTitleClassAndText("btn-title-grey", "Total 0 zBIC");
    ui->btnTotalzBIC->setSubTitleClassAndText("text-subtitle", "Show denominations of zBIC owned.");
    ui->btnTotalzBIC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BIC outputs to mint into zBIC.");
    ...
    connect(ui->btnTotalzBIC, SIGNAL(clicked()), this, SLOT(onTotalZbicClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBIC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BIC to mint into zBIC"));
    }else{
        btnText = tr("Convert back to BIC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBIC to convert back into BIC"));
    }
...
void PrivacyWidget::onTotalZbicClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzBIC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBIC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBIC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBIC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBIC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBIC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZbic(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zBIC converted back to BIC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBIC </b>";
    ...
    ui->btnTotalzBIC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
