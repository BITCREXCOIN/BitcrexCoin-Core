...
#include "qt/bitcrexcoin/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/bitcrexcoin/forms/ui_topbar.h"
#include "qt/bitcrexcoin/lockunlock.h"
#include "qt/bitcrexcoin/qtutils.h"
#include "qt/bitcrexcoin/receivedialog.h"
#include <QPixmap>
...
TopBar::TopBar(BITCREXCOINGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                             ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    foreach (QWidget* w, lblTitles) {
        w->setFont(font);
    }

    // Amount information top
    ui->widgetTopAmount->setVisible(false);
    setCssProperty({ui->labelAmountTopBic, ui->labelAmountTopzBic}, "amount-small-topbar");
    setCssProperty({ui->labelAmountBic, ui->labelAmountzBic}, "amount-topbar");
    setCssProperty({ui->labelPendingBic, ui->labelPendingzBic, ui->labelImmatureBic, ui->labelImmaturezBic}, "amount-small-topbar");

    ui->labelAmountTopzBic->setVisible(false);
    ui->label_16->setVisible(false);
    ui->labelTitle2->setVisible(false);
    ui->labelAmountzBic->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzBic->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezBic->setVisible(false);
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("BITCREXCOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& zerocoinBalance, const CAmount& unconfirmedZerocoinBalance, const CAmount& immatureZerocoinBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    CAmount bicAvailableBalance = balance + delegatedBalance - nLockedBalance;
    ...
    ui->labelAmountTopzBic->setText(totalzBic);

    // Expanded
    ui->labelAmountBic->setText(totalBic);
    ui->labelAmountzBic->setText(totalzBic);

    ui->labelPendingBic->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzBic->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureBic->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezBic->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
