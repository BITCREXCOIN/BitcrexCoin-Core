...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BIC);
    ...
    if (settings.contains("nAnonymizeBitcrexCoinAmount") || reset)
        SoftSetArg("-anonymizebitcrexcoinamount", settings.value("nAnonymizeBitcrexCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitcrexcoin.cpp during shotdown to also remove the window positions
