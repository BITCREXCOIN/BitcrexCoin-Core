...
CZBicStake::CZBicStake(const libzerocoin::CoinSpend& spend)
{
...
int CZBicStake::GetChecksumHeightFromMint()
{
...
int CZBicStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZBicStake::GetChecksum()
{
    return nChecksum;
}

// The zBIC block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZBicStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZBicStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zBIC staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZBicStake::GetUniqueness()
{
    //The unique identifier for a zBIC is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZBicStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZBicStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zBIC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBICOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBIC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBIC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBICOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBIC output", __func__);
...
bool CZBicStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZBicStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzBICTracker* zbicTracker = pwallet->zbicTracker.get();
    CMintMeta meta;
    if (!zbicTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbicTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!BIC Stake
bool CBicStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBicStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CBicStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBicStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CBicStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CBicStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CBicStake::GetUniqueness()
{
    //The unique identifier for a BIC stake is the outpoint
...
CBlockIndex* CBicStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
