...
public:
    ...
    CAmount GetAccountBicDebit(const std::string& strAccount);
    void ListAccountBicDebit(const std::string& strAccount, std::list<CAccountingEntry>& acentries);
    ...
    bool WriteZBICSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZBICSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZBICSeed_deprecated(uint256& seed);
    bool EraseZBICSeed();
    bool EraseZBICSeed_deprecated();

    bool WriteZBICCount(const uint32_t& nCount);
    bool ReadZBICCount(uint32_t& nCount);
