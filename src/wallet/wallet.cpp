...
// Copyright (c) 2015-2019 The BITCREXCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "wallet/wallet.h"

#include "zbic/accumulators.h"
#include "base58.h"
...
#include "zbic/zbicwallet.h"
#include "zbic/zbictracker.h"
#include "zbic/deterministicmint.h"
#include <assert.h>
...
/**
 * Fees smaller than this (in ubic) are considered zero fee (for transaction creation)
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwalletdb)
{
    ...
    if (fFromLoadWallet) {
        ...
        // Break debit/bic balance caches:
        wtx.MarkDirty();
...
{
    return zbicTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetUnspentBic(const isminefilter& filter) const
{
    ...
    CAmount bic = 0;
    if (filter & ISMINE_SPENDABLE) {
        bic += pwallet->GetBic(*this, ISMINE_SPENDABLE, true);
    }
    if (filter & ISMINE_WATCH_ONLY) {
        bic += pwallet->GetBic(*this, ISMINE_WATCH_ONLY, true);
    }
    if (filter & ISMINE_COLD) {
        bic += pwallet->GetBic(*this, ISMINE_COLD, true);
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        bic += pwallet->GetBic(*this, ISMINE_SPENDABLE_DELEGATED, true);
    }
    return bic;
}

CAmount CWalletTx::GetBic(const isminefilter& filter) const
{
    CAmount bic = 0;
    if (filter & ISMINE_SPENDABLE) {
        ...
        if (fBicCached)
            ...
            nBicCached = pwallet->GetBic(*this, ISMINE_SPENDABLE, false);
            fBicCached = true;
            bic += nBicCached;
        }
    }
    if (filter & ISMINE_WATCH_ONLY) {
        if (fWatchBicCached)
            ...
            nWatchBicCached = pwallet->GetBic(*this, ISMINE_WATCH_ONLY, false);
            fWatchBicCached = true;
            bic += nWatchBicCached;
        }
    }
    if (filter & ISMINE_COLD) {
        if (fColdBicCached)
            ...
            nColdBicCached = pwallet->GetBic(*this, ISMINE_COLD, false);
            fColdBicCached = true;
            bic += nColdBicCached;
        }
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        if (fDelegatedBicCached)
            bic += nDelegatedBicCached;
        else {
            nDelegatedBicCached = pwallet->GetBic(*this, ISMINE_SPENDABLE_DELEGATED, false);
            fDelegatedBicCached = true;
            bic += nDelegatedBicCached;
        }
    }
    return bic;
...
CAmount CWalletTx::GetImmatureBic(bool fUseCache, const isminefilter& filter) const
{
    LOCK(cs_main);
    if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureBicCached && filter == ISMINE_SPENDABLE_ALL)
            return nImmatureBicCached;
        nImmatureBicCached = pwallet->GetBic(*this, filter);
        fImmatureBicCached = true;
        return nImmatureBicCached;
    }
...
CAmount CWalletTx::GetAvailableBic(bool fUseCache) const
{
    return GetUnspentBic(ISMINE_SPENDABLE_ALL);
}

CAmount CWalletTx::GetColdStakingBic(bool fUseCache) const
{
    return GetUnspentBic(ISMINE_COLD);
}

CAmount CWalletTx::GetStakeDelegationBic(bool fUseCache) const
{
    return GetUnspentBic(ISMINE_SPENDABLE_DELEGATED);
}

CAmount CWalletTx::UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fBic) const
{
    if (pwallet == 0)
        return 0;
    ...
    amountToUpdate = (fBic) ? GetBic(mimeType) : GetDebit(mimeType);
    cacheFlagToUpdate = true;
    return amountToUpdate;
}

// Return sum of unlocked coins
CAmount CWalletTx::GetUnlockedBic() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        nBic += pwallet->GetBic(txout, ISMINE_SPENDABLE);
        if (!MoneyRange(nBic))
            throw std::runtime_error("CWalletTx::GetUnlockedBic() : value out of range");
    }
...
CAmount CWalletTx::GetLockedBic() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add delegated coins
        nBic += pwallet->GetBic(txout, ISMINE_SPENDABLE_DELEGATED);
        ...
        if (!MoneyRange(nBic))
            throw std::runtime_error("CWalletTx::GetLockedBic() : value out of range");
    }

    return nBic;
}

CAmount CWalletTx::GetImmatureWatchOnlyBic(const bool& fUseCache) const
{
    LOCK(cs_main);
    if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureWatchBicCached)
            return nImmatureWatchBicCached;
        nImmatureWatchBicCached = pwallet->GetBic(*this, ISMINE_WATCH_ONLY);
        fImmatureWatchBicCached = true;
        return nImmatureWatchBicCached;
    }
...
CAmount CWalletTx::GetAvailableWatchOnlyBic(const bool& fUseCache) const
{
    if (pwallet == 0)
        return 0;
    ...
    if (fUseCache && fAvailableWatchBicCached)
        return nAvailableWatchBicCached;

    CAmount nBic = 0;
    for (unsigned int i = 0; i < vout.size(); i++) {
        if (!pwallet->IsSpent(GetHash(), i)) {
            const CTxOut& txout = vout[i];
            nBic += pwallet->GetBic(txout, ISMINE_WATCH_ONLY);
            if (!MoneyRange(nBic))
                throw std::runtime_error("CWalletTx::GetAvailableBic() : value out of range");
        }
    }

    nAvailableWatchBicCached = nBic;
    fAvailableWatchBicCached = true;
    return nBic;
}

CAmount CWalletTx::GetLockedWatchOnlyBic() const
{
    if (pwallet == 0)
        return 0;
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZBIC = GetBoolArg("-zapwallettxes", false);
    if (fCheckZBIC)
        zbicTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zbic
            if (fCheckZBIC && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetColdStakingBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
        if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
            nTotal += pcoin.GetColdStakingBic();
    });
}
...
CAmount CWallet::GetDelegatedBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
                nTotal += pcoin.GetStakeDelegationBic();
    });
}
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zBIC in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zbicTracker->GetMints(true);
    ...
    return zbicTracker->GetBalance(false, false);
}
...
{
    return zbicTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zbicTracker->ListMints(true, true, true);
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureBic(false);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureBic(false, ISMINE_COLD);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureBic(false, ISMINE_SPENDABLE_DELEGATED);
    });
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted())
                nTotal += pcoin.GetAvailableWatchOnlyBic();
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureWatchOnlyBic();
    });
}
...
CAmount CWallet::GetLockedWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted() && pcoin.GetDepthInMainChain() > 0)
                nTotal += pcoin.GetLockedWatchOnlyBic();
    });
}
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-bicstake", true) && !fPrecompute) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CBicStake> input(new CBicStake());
            input->SetInput((CTransaction) *out.tx, out.i);
            listInputs.emplace_back(std::move(input));
        }
    }

    /* Disable zBIC Staking
    //zBIC
    if ((GetBoolArg("-zbicstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zBIC set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = zbicTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZbicBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 BIC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 BIC collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 500 BIC.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 500 BIC.");
                    } else {
                ...
                for (PAIRTYPE(const CWalletTx*, unsigned int) pcoin : setCoins) {
                    ...
                    CAmount nBic = pcoin.first->vout[pcoin.second].nValue;
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-bitcrexcoin-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            if (!stakeInput->CreateTxOuts(this, vout, nBic)) {
            ...
            CAmount nMinFee = 0;
            if (!stakeInput->IsZBIC()) {
                // Set output amount
                int outputs = txNew.vout.size() - 1;
                CAmount nRemaining = nBic - nMinFee;
                if (outputs > 1) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZBIC());

            {
                TRY_LOCK(zbicTracker->cs_spendcache, fLocked);
                if (!fLocked)
                    continue;
            ...
            //Mark mints as spent
            if (stakeInput->IsZBIC()) {
                CZBicStake* z = (CZBicStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zBIC. Current percentage of zBIC: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zBIC are integers > 0, so we can't mint 10% of 9 BIC
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zBIC\n", nBalance);
        return;
    }

    // Percentage of zBIC we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zBIC (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zBIC amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zBIC amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zBIC We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough BIC to mint this denomination
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            nAmount = ((out.tx->GetBic(filter) - out.tx->GetDebit(filter)) * vMultiSend[i].second) / 100;
...
bool CWallet::CreateZBICOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZBIC(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZBICOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zbic output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Bic
    ...
    // Sign if these are bitcrexcoin outputs - NOTE that zBIC outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZBIC_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zBIC
        receipt.SetStatus(_("The coin spend has been used"), ZBIC_SPENT_USED_ZBIC);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zbicTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zbicTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zbicTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZBIC_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zbicTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zbicTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zbicTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZBIC_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zBIC privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZBIC_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZBIC_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZBIC_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZBIC_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZBIC_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZBIC_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZBIC_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZBICModule::createInput(in, mint, hashTxOut, spendVersion)) {
                receipt.SetStatus(_("Cannot create public spend input"), ZBIC_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZBIC_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zBIC"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZBIC_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zBIC public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zBIC used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zbicTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zBIC mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to bitcrexcoin address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZBIC_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZBIC_SPEND_OKAY); // Everything okay
...
std::string CWallet::ResetMintZerocoin()
{
    ...
    std::set<CMintMeta> setMints = zbicTracker->ListMints(false, false, true);
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zbicTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zbicTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zbicTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zbicTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzbicAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzbicAuto ? "-autozbicbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZBicBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozbicbackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozbicbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozbicbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zbicbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zbicbackuppath", ""));
...
)
{
    ...
    int nStatus = ZBIC_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZBIC_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZBIC_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zbicTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZBIC_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZBIC_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zbicTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zbicTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZBIC_SPEND_OKAY);  // When we reach this point spending zBIC was successful
...
{
    CMintMeta meta;
    if (!zbicTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zbicTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zbicTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zbicTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zbicTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zbicTracker->Get(hashSerial);
    zbicTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
...
{
    // We don't even need to worry about this code.. no zBIC.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("bitcrexcoin-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zbicTracker->GetSpendCache(serialHash);
...
void CWallet::setZWallet(CzBICWallet* zwallet)
{
    zwalletMain = zwallet;
    zbicTracker = std::unique_ptr<CzBICTracker>(new CzBICTracker(strWalletFile));
}

CzBICWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZBicAutoBackups(bool fEnabled)
{
...
CAmount CWallet::GetBic(const CTxOut& txout, const isminefilter& filter) const
{
    if (!MoneyRange(txout.nValue))
        throw std::runtime_error("CWallet::GetBic() : value out of range");
...
CAmount CWallet::GetBic(const CTransaction& tx, const isminefilter& filter, const bool fUnspent) const
{
    CAmount nBic = 0;
    for (int i = 0; i < tx.vout.size(); i++) {
        if (fUnspent && IsSpent(tx.GetHash(), i)) continue;
        nBic += GetBic(tx.vout[i], filter);
    }
    if (!MoneyRange(nBic))
        throw std::runtime_error("CWallet::GetBic() : value out of range");
    return nBic;
...
void CWalletTx::Init(const CWallet* pwalletIn)
{
    ...
    fDebitCached = false;
    fBicCached = false;
    fImmatureBicCached = false;
    fAvailableBicCached = false;
    fAnonymizableBicCached = false;
    fAnonymizedBicCached = false;
    fDenomUnconfBicCached = false;
    fDenomConfBicCached = false;
    fWatchDebitCached = false;
    fWatchBicCached = false;
    fImmatureWatchBicCached = false;
    fAvailableWatchBicCached = false;
    fChangeCached = false;
    fColdDebitCached = false;
    fColdBicCached = false;
    fDelegatedDebitCached = false;
    fDelegatedBicCached = false;
    fStakeDelegationVoided = false;
    nDebitCached = 0;
    nBicCached = 0;
    nImmatureBicCached = 0;
    nAvailableBicCached = 0;
    nAnonymizableBicCached = 0;
    nAnonymizedBicCached = 0;
    nDenomUnconfBicCached = 0;
    nDenomConfBicCached = 0;
    nWatchDebitCached = 0;
    nWatchBicCached = 0;
    nAvailableWatchBicCached = 0;
    nImmatureWatchBicCached = 0;
    nColdDebitCached = 0;
    nColdBicCached = 0;
    nDelegatedDebitCached = 0;
    nDelegatedBicCached = 0;
    nChangeCached = 0;
    nOrderPos = -1;
}
