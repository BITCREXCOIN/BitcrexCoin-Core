...
CAmount CWalletDB::GetAccountBicDebit(const std::string& strAccount)
{
    std::list<CAccountingEntry> entries;
    ListAccountBicDebit(strAccount, entries);

    CAmount nBicDebit = 0;
    for (const CAccountingEntry& entry : entries)
        nBicDebit += entry.nBicDebit;

    return nBicDebit;
}

void CWalletDB::ListAccountBicDebit(const std::string& strAccount, std::list<CAccountingEntry>& entries)
{
    bool fAllAccounts = (strAccount == "*");

    Dbc* pcursor = GetCursor();
    if (!pcursor)
        throw std::runtime_error("CWalletDB::ListAccountBicDebit() : cannot create DB cursor");
    unsigned int fFlags = DB_SET_RANGE;
    while (true) {
        // Read next record
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        ...
        if (ret == DB_NOTFOUND)
            ...
            pcursor->close();
            throw std::runtime_error("CWalletDB::ListAccountBicDebit() : error scanning DB");
        }
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    ListAccountBicDebit("*", pwallet->laccentries);
...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("bitcrexcoin-wallet");
...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(std::make_pair(std::string("dzbic"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(std::make_pair(std::string("dzbic"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(std::make_pair(std::string("dzbic"), hashPubcoin));
}
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(std::make_pair(std::string("dzbic"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteZBICSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZBICSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZBICSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZBICSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZBICSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZBICSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZBICCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZBICCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(std::string("dzbic"), uint256(0));
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
