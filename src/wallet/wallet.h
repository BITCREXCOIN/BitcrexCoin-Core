...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zbicspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 500 BIC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBIC send
enum ZerocoinSpendStatus {
    ZBIC_SPEND_OKAY = 0,                            // No error
    ZBIC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBIC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBIC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBIC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBIC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBIC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBIC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBIC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBIC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBIC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBIC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBIC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBIC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBIC_SPENT_USED_ZBIC = 14,                      // Coin has already been spend
    ZBIC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBIC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZBicBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBICOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzbicAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBICTracker> zbicTracker;
    ...
    void setZWallet(CzBICWallet* zwallet);
    CzBICWallet* getZWallet();
    bool isZeromintEnabled();
    void setZBicAutoBackups(bool fEnabled);
    ...
    /// Get 500 BIC output and keys which can be used for the Masternode
    ...
    CAmount GetBic(const CTxOut& txout, const isminefilter& filter) const;
    ...
    CAmount GetBic(const CTransaction& tx, const isminefilter& filter, const bool fUnspent = false) const;
    ...
    /** zBIC reset */
    boost::signals2::signal<void()> NotifyzBICReset;
...
public:
    ...
    mutable bool fBicCached;
    mutable bool fImmatureBicCached;
    mutable bool fAvailableBicCached;
    mutable bool fAnonymizableBicCached;
    mutable bool fAnonymizedBicCached;
    mutable bool fDenomUnconfBicCached;
    mutable bool fDenomConfBicCached;
    mutable bool fWatchDebitCached;
    mutable bool fWatchBicCached;
    mutable bool fImmatureWatchBicCached;
    mutable bool fAvailableWatchBicCached;
    mutable bool fChangeCached;
    mutable bool fColdDebitCached;
    mutable bool fColdBicCached;
    mutable bool fDelegatedDebitCached;
    mutable bool fDelegatedBicCached;
    mutable bool fStakeDelegationVoided;
    mutable CAmount nDebitCached;
    mutable CAmount nBicCached;
    mutable CAmount nImmatureBicCached;
    mutable CAmount nAvailableBicCached;
    mutable CAmount nAnonymizableBicCached;
    mutable CAmount nAnonymizedBicCached;
    mutable CAmount nDenomUnconfBicCached;
    mutable CAmount nDenomConfBicCached;
    mutable CAmount nWatchDebitCached;
    mutable CAmount nWatchBicCached;
    mutable CAmount nImmatureWatchBicCached;
    mutable CAmount nAvailableWatchBicCached;
    mutable CAmount nChangeCached;
    mutable CAmount nColdDebitCached;
    mutable CAmount nColdBicCached;
    mutable CAmount nDelegatedDebitCached;
    mutable CAmount nDelegatedBicCached;
    ...
    CAmount GetBic(const isminefilter& filter) const;
    CAmount GetUnspentBic(const isminefilter& filter) const;
    CAmount GetImmatureBic(bool fUseCache = true, const isminefilter& filter = ISMINE_SPENDABLE_ALL) const;
    CAmount GetAvailableBic(bool fUseCache = true) const;
    // Return sum of unlocked coins
    CAmount GetUnlockedBic() const;
    // Return sum of unlocked coins
    CAmount GetLockedBic() const;
    CAmount GetImmatureWatchOnlyBic(const bool& fUseCache = true) const;
    CAmount GetAvailableWatchOnlyBic(const bool& fUseCache = true) const;
    CAmount GetLockedWatchOnlyBic() const;
    CAmount GetChange() const;

    // Cold staking contracts bic/debit
    CAmount GetColdStakingBic(bool fUseCache = true) const;
    CAmount GetColdStakingDebit(bool fUseCache = true) const;
    CAmount GetStakeDelegationBic(bool fUseCache = true) const;
    CAmount GetStakeDelegationDebit(bool fUseCache = true) const;

    // Helper method to update the amount and cacheFlag.
    CAmount UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fBic = true) const;
