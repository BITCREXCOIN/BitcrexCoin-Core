...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new BITCREXCOIN address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be biced to 'account'.\n"

            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"

            "\nResult:\n"
            "\"bitcrexcoinaddress\"    (string) The new bitcrexcoin address\n"
...
UniValue getnewstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new BITCREXCOIN cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const UniValue& params, bool fHelp)
{
    ...
    if (!address.IsValid() || address.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BITCREXCOIN address");

    CKeyID keyID;
    if (!address.GetKeyID(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from BITCREXCOIN address");
...
UniValue delegatorremove(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get BITCREXCOIN address from addressBook");
...
UniValue listdelegators(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) BITCREXCOIN address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current BITCREXCOIN address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"bitcrexcoinaddress\"   (string) The account bitcrexcoin address\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new BITCREXCOIN address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"bitcrexcoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"bitcrexcoinaddress\"  (string, required) The bitcrexcoin address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"bitcrexcoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"bitcrexcoinaddress\"  (string, required) The bitcrexcoin address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"bitcrexcoinaddress\"  (string) a bitcrexcoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"bitcrexcoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"bitcrexcoinaddress\"  (string, required) The bitcrexcoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in BIC to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!stakeAddr.IsValid() || !stakeAddr.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BITCREXCOIN staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!ownerAddr.IsValid() || ownerAddr.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BITCREXCOIN spending address");
...
UniValue delegatestake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The bitcrexcoin staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in BIC to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The bitcrexcoin address corresponding to the key that will be able to spend the stake. \n"
...
UniValue rawdelegatestake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "           \"bitcrexcoinaddress\"        (string) bitcrexcoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"bitcrexcoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"bitcrexcoinaddress\",     (string) The bitcrexcoin address\n"
            "      amount,                 (numeric) The amount in BIC\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"bitcrexcoinaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"bitcrexcoinaddress\"  (string, required) The bitcrexcoin address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"bitcrexcoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given bitcrexcoinaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"bitcrexcoinaddress\"  (string, required) The bitcrexcoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in BIC received at this address.\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BIC received for this account.\n"
...
CAmount GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth, const isminefilter& filter)
{
    ...
    if (strAccount != "*")
        nBalance += walletdb.GetAccountBicDebit(strAccount);
...
UniValue getcoldstakingbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BIC received for this account in P2CS contracts.\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of BIC to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 BIC from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 BIC from timotei to akiko with a comment\n" +
    ...
    // Bic
    CAccountingEntry bic;
    bic.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    bic.strAccount = strTo;
    bic.nBicDebit = nAmount;
    bic.nTime = nNow;
    bic.strOtherAccount = strFrom;
    bic.strComment = strComment;
    pwalletMain->AddAccountingEntry(bic, walletdb);
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"tobitcrexcoinaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nSent an amount from an account to a bitcrexcoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tobitcrexcoinaddress\"  (string, required) The bitcrexcoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in BIC. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 BIC from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The bitcrexcoin address is the key, the numeric amount in BIC is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!address.IsValid() || address.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BITCREXCOIN address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a BITCREXCOIN address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of bitcrexcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) bitcrexcoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"bitcrexcoinaddress\"  (string) A bitcrexcoin address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in BIC received by the address\n"
...
UniValue listcoldutxos(const UniValue& params, bool fHelp)
{
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it =
        ...
        if(pcoin->GetColdStakingBic() == 0 && pcoin->GetStakeDelegationBic() == 0)
            continue;
...
void AcentryToJSON(const CAccountingEntry& acentry, const std::string& strAccount, UniValue& ret)
{
    ...
    if (fAllAccounts || acentry.strAccount == strAccount) {
        ...
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nBicDebit)));
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 6)
        throw std::runtime_error(
            ...
            "    \"address\":\"bitcrexcoinaddress\",    (string) The bitcrexcoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BIC. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in BIC. This is negative and only available for the \n"
...
UniValue listaccounts(const UniValue& params, bool fHelp)
{
    ...
    for (const CAccountingEntry& entry : acentries)
        mapAccountBalances[entry.strAccount] += entry.nBicDebit;
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"bitcrexcoinaddress\",    (string) The bitcrexcoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BIC. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in BIC. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in BIC\n"
            ...
            "      \"address\" : \"bitcrexcoinaddress\",   (string) The bitcrexcoin address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in BIC\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
    ...
    CAmount nBic = wtx.GetBic(filter);
    CAmount nDebit = wtx.GetDebit(filter);
    CAmount nNet = nBic - nDebit;
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BICs\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending BICs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"bitcrexcoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; bitcrexcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending BICs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in BIC/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total BIC balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the BIC balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the BIC balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in BIC\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in BIC\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in BIC\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in BIC\n"
            "  \"txcount\": xxxxxxx,                      (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,                 (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,                     (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,                   (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,                      (numeric) the transaction fee configuration, set in BIC/kB\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same BITCREXCOIN address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "multisend <BITCREXCOIN Address> <percent>\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BIC address");
...
//         throw std::runtime_error(
//             "getzerocoinbalance\n"
//             "\nReturn the wallet's total zBIC balance.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nResult:\n"
//             "amount         (numeric) Total zBIC balance.\n"
...
//             "\nList all zBIC mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "    \"zBIC ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zbicTracker->ListMints(true, fMatureOnly, true);
...
//             objMint.push_back(Pair("zBIC ID", m.hashPubcoin.GetHex()));     // PubCoin
...
//                     pwalletMain->zbicTracker->UpdateState(m);
//                 }
//             }
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zbicTracker->ListMints(true, true, true);
...
//         throw std::runtime_error(
//             "listspentzerocoins\n"
//             "\nList all the spent zBIC mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "\nMint the specified zBIC amount\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. amount      (numeric, required) Enter an amount of Bic to convert to zBIC\n"
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zBIC minting is DISABLED");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zBIC is currently disabled due to maintenance.");
...
//             "    {\n"
//             "      \"value\": amount,         (numeric) Value in BIC.\n"
//             "      \"address\": \"xxx\"         (string) BIC address or \"zerocoinmint\" for reminted change.\n"
//             "    }\n"
//             "    ,...\n"
//             "  ]\n"
//             "}\n"
...
//     bool fMintChange = params[1].get_bool();        // Mint change to zBIC
//     if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
//         throw JSONRPCError(RPC_WALLET_ERROR, "zBIC minting is DISABLED, cannot mint change");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zBIC old spend only available in regtest for tests purposes");
//     }

//     return DoZbicSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
// }
...
//             "\nSpend zBIC mints to a BIC address.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//     return DoZbicSpend(nAmount, false, true, vMintsSelected, address_str);
// }


// extern UniValue DoZbicSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
// {
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzBICTracker* zbicTracker = pwalletMain->zbicTracker.get();
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzBICTracker* zbicTracker = pwalletMain->zbicTracker.get();
//     std::set<CMintMeta> setMints = zbicTracker->ListMints(false, false, false);
...
//             "2. \"denomination\"         (integer, optional) Export a specific denomination of zBIC\n"

//             "\nResult:\n"
//             "[                   (array of json object)\n"
//             "  {\n"
//             "    \"id\": \"serial hash\",  (string) the mint's zBIC serial hash \n"
...
//             "    \"v\": version,   (numeric) The version of the zBIC\n"
//             "    \"k\": \"privkey\"  (string) The zBIC private key (V2+ zBIC only)\n"
//             "  }\n"
//             "  ,...\n"
//             "]\n"
...
//     CzBICTracker* zbicTracker = pwalletMain->zbicTracker.get();
//     std::set<CMintMeta> setMints = zbicTracker->ListMints(!fIncludeSpent, false, false);
...
//             "  \"value\": amount    (numeric) The total zBIC value of zerocoin mints that were added\n"
//             "}\n"
...
//         pwalletMain->zbicTracker->Add(mint, true);
//         count++;
...
//         throw std::runtime_error(
//             "reconsiderzerocoins\n"
//             "\nCheck archived zBIC list to see if any mints were added to the blockchain.\n" +
//             HelpRequiringPassphrase() + "\n"
...
// UniValue setzbicseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 1)
//         throw std::runtime_error(
//             "setzbicseed \"seed\"\n"
//             "\nSet the wallet's deterministic zbic seed to a specific value.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. \"seed\"        (string, required) The deterministic zbic seed.\n"

//             "\nResult\n"
//             "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

//             "\nExamples\n" +
//             HelpExampleCli("setzbicseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
//             HelpExampleRpc("setzbicseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
...
//     CzBICWallet* zwallet = pwalletMain->getZWallet();
...
// UniValue getzbicseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || !params.empty())
//         throw std::runtime_error(
//             "getzbicseed\n"
...
//             "\nResult\n"
//             "\"seed\" : s,  (string) The deterministic zBIC seed.\n"

//             "\nExamples\n" +
//             HelpExampleCli("getzbicseed", "") + HelpExampleRpc("getzbicseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zBIC seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zBIC to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zBIC to generate.\n"
    ...
    CzBICWallet* zwallet = pwalletMain->zwalletMain;
...
// UniValue dzbicstate(const UniValue& params, bool fHelp) {
//     if (fHelp || params.size() != 0)
//         throw std::runtime_error(
//                 "dzbicstate\n"
//                         "\nThe current state of the mintpool of the deterministic zBIC wallet.\n" +
//                 HelpRequiringPassphrase() + "\n"
...
//     obj.push_back(Pair("dzbic_count", nCount));
...
void static SearchThread(CzBICWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZBIC(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
// UniValue searchdzbic(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 3)
//         throw std::runtime_error(
//             "searchdzbic\n"
//             "\nMake an extended search for deterministically generated zBIC that have not yet been recognized by the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments\n"
//             "1. \"count\"       (numeric) Which sequential zBIC to start with.\n"
//             "2. \"range\"       (numeric) How many zBIC to generate.\n"
//             "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

//             "\nExamples\n" +
//             HelpExampleCli("searchdzbic", "1, 100, 2") + HelpExampleRpc("searchdzbic", "1, 100, 2"));
...
//     boost::thread_group* dzbicThreads = new boost::thread_group();
...
//         nPrevThreadEnd = nEnd;
//         dzbicThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
//     }

//     dzbicThreads->join_all();

//     zwallet->RemoveMintsFromPool(pwalletMain->zbicTracker->GetSerialHashes());
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) BITCREXCOIN address for auto minting\n" +
...
//             "5. \"address\"          (string, optional) BITCREXCOIN address to spend to. If not specified, spend to change add.\n"
...
//         throw std::runtime_error(
//             "clearspendcache\n"
//             "\nClear the pre-computed zBIC spend cache, and database.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             if (fLocked) {
//                 if (zbicTracker->ClearSpendCache()) {
