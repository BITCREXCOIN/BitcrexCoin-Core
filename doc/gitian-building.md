...
*Setup instructions for a gitian build of BITCREXCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BITCREXCOIN
...
to the BITCREXCOIN GitHub release page.
...
- [Building BITCREXCOIN Core](#building-bitcrexcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bitcrexcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bitcrexcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bitcrexcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bitcrexcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bitcrexcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bitcrexcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bitcrexcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bitcrex.xyz) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bitcrexcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bitcrexcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bitcrexcoin-osx-*-build.assert
```
