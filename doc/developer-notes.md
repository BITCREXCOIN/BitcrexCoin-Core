...
- [Development guidelines](#development-guidelines)
    - [General BITCREXCOIN Core](#general-bitcrexcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BITCREXCOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BICs (tBIC)" on the test network, if you
...
BITCREXCOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bitcrexcoin-project/bitcrexcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitcrexcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitcrexcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/bitcrexcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bitcrexcoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates BICs (if wallet is enabled).
...
However, in open source software such as BITCREXCOIN Core, where everyone uses
...
pay attention to for reviewers of BITCREXCOIN Core code.

General BITCREXCOIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BITCREXCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BITCREXCOIN Core so that they can be integrated
...
file descriptors it will cause problems with BITCREXCOIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:BITCREXCOIN-Project/BITCREXCOIN.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitcrexcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitcrexcoin-cli` and the GUI debug console use this table to determine how to
